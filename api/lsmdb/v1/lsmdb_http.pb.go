...
func _Lsmdb_CloseDB0_HTTP_Handler(srv LsmdbHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		...
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.CloseDB(ctx, req.(*CloseRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
type LsmdbHTTPClient interface {
	CloseDB(ctx context.Context, req *CloseRequest, opts ...http.CallOption) (rsp *CloseReply, err error)
...
func (c *LsmdbHTTPClientImpl) CloseDB(ctx context.Context, in *CloseRequest, opts ...http.CallOption) (*CloseReply, error) {
	var out CloseReply
	pattern := "/lsmdb/close"
